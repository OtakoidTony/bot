        ...
        if message.content.startswith('키키야'):
            ...
            if message.content == "키키야": await message.channel.send("안녕하세요 키키봇입니다^^")
                ...
                if message.author.id==647630912795836437:
                    ...
                    await author.send(f'**{message.author.display_name}**님이 **{str(why)}**라는 이유 때문에 님을 차단하셨습니다')
                    await message.channel.send('성공적으로 차단 완료')
                else:
                    await message.channel.send('권한이 없네여')
            elif message.content.startswith('키키야 건의'):
                learn=message.content[7:]
                author=message.guild.get_member(647630912795836437)
                await author.send(f'{message.author.display_name}님의 건의사항: {str(learn)}')
                await message.channel.send('건의사항이 성공적으로 전송되었습니다')
            elif message.content=='키키야 맬론차트':
                if __name__=="__main__":
                    ...
                    for i in range(RANK):
                        embed.add_field(name='%3d위'%(i+1), value='%s - %s'%(title[i], song[i]))
                    await message.channel.send(embed=embed)
                ...
                while sheet["A" + str(i)].value != None:
                    i += 1
                    if str(sheet["A" + str(i)].value) == str(message.author.id):
                        if int(sheet['C'+str(i)].value)<int(money):
                            await message.channel.send('돈이나 벌고 오세여')
                            return
                        await message.channel.send('성공하면 그 돈의 2배를 얻지만 실패하면 그 돈의 양만큼 뺏깁니다')
                        rand=random.randrange(1, 3)
                        if rand==1:
                            await message.channel.send('실패하셨습니다!!\n~~역시 도박은 몸에 안좋아여~~')
                            ohyeah=int(sheet['C'+str(i)].value)-int(money)
                            sheet['C'+str(i)].value=str(ohyeah)
                            file.save('정보.xlsx')
                        else:
                            await message.channel.send('성공하셨습니다!!\n~~역시 도박은 몸에 좋아여~~')
                            ohyeah=int(sheet['C'+str(i)].value)+int(money)*2
                            sheet['C'+str(i)].value=str(ohyeah)
                            file.save('정보.xlsx')
                        return
                await message.channel.send('`키키야 가입`을 입력해 가입을 먼저 하세여')        
                ...
                while sheet["A" + str(i)].value != None:
                    ...
                    if str(sheet["A" + str(i)].value) == str(message.author.id):
                        ...
                        break
                if flag == 0 : await message.channel.send("`키키야 가입`을 입력해 먼저 가입을 하세요")
            elif message.content=='키키야 명령어초기화':
                if message.author.id==647630912795836437:
                    i=1
                    ...
                    while sheet['A'+str(i)].value!=None:
                        ...
                        sheet['D'+str(i)].value=''
                        i+=1
                    await message.channel.send('초기화 성공')
                    file.save('기억.xlsx')
                else: await message.channel.send('에휴, 관리자나 얻고 오세여')
            elif message.content=='키키야 명령어확인':
                file = openpyxl.load_workbook("기억.xlsx")
                sheet = file.active
                i=1
                embed=discord.Embed(
                ...
                if flag!=0: 
                    if j>1:
                        ...
                        await message.channel.send(str(sheet['B'+str(learn[rand-1])].value))
                    else:
                        await message.channel.send(str(sheet['B'+str(learn[0])].value))
                else: 
                    await message.channel.send("키키봇의 프로그램에는 없는 명령어에요!")
